...
@click.option(
    ...
    help="The database version to migrate to.",
    type=int,
)
def download_latest_db(db_version: int):
    ...
    db_versions_request = s3_client.list_objects_v2(
        Bucket="daily-dice-roller-db-backup-pro",
        Prefix=f"backups/env={env}/version={CURRENT_DB_VERSION}/",
    )
    ...
    print(
        f'Found {db_versions_request["KeyCount"]} db versions. Sorting and getting latest...'
    )
    # Sort the keys by the date in the object name
    db_versions = sorted(
        db_versions_request["Contents"], key=lambda x: x["Key"], reverse=True
    )
    ...
    s3_client.download_file(
        "daily-dice-roller-db-backup-pro", latest_version.key, local_path
    )
