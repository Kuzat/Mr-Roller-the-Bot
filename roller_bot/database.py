...
class RollDatabase:
    ...
    async def backup(self, user: discord.User) -> None:
        ...
        await user.send(
            content=f"[{current_time}]: Backup of database {self.db_path}",
            file=discord.File(self.db_path),
        )
    ...
    async def backup_to_s3(self, user: discord.User) -> None:
        ...
        s3_path = RollDatabase.backup_s3(
            self.db_path,
            self.s3_client,
            env,
            db_version=self.db_path.split("_v")[-1].split(".")[0],
        )
    ...
    def backup_s3(db_path: str, s3_client, env: str, db_version: int) -> str:
        ...
        s3_path = (
            f"backups/env={env}/version={db_version}/{datetime.now().isoformat()}.db"
        )
